...
**`target_zone` and the zone sensor update at different points, by design, not by lag.** `target_zone` (type 3, `partitionIds`) is set the moment a mow-start command is issued — it's the assigned job, so it can already be visible while the mower is still docked or paused, before it has moved at all. The zone sensor (type 2, `currentMowBoundary`) only updates once the mower physically crosses into that zone — live-observed delay of ~30-40s between a start command and the zone sensor leaving `unknown`, because the mower first has to drive to and cross the zone boundary. If you're building an automation that reacts to the current zone, account for this start-up gap; `target_zone` tells you what was *commanded*, the zone sensor tells you where the mower *physically is*.
